...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("read_a_section_as_clean_text",
			mcplib.WithDescription("Fetches a chapter/section chunk and returns each section's title and HTML-stripped plaintext."),
			mcplib.WithString("slug", mcplib.Required(), mcplib.Description("Override the recipe's positional slug value.")),
		),
		handleReadASectionAsCleanText,
	)
}

// handleReadASectionAsCleanText runs the read_a_section_as_clean_text recipe intent: Fetches a chapter/section chunk and returns each section's title and HTML-stripped plaintext.
func handleReadASectionAsCleanText(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
