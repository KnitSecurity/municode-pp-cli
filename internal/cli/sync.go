...
Resource scoping:
  ...
  municode-pp-cli sync

  # Sync specific resources only
  municode-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  municode-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  municode-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  municode-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  municode-pp-cli sync --latest-only`,
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"clients":             "/Clients/stateAbbr",
		"codestoc":            "/codesToc/children",
		"codestoc-breadcrumb": "/codesToc/breadcrumb",
		"states":              "/States",
	}
...
// flat paths.
// Municode resources use PascalCase "<Type>ID" primary keys that the generic
// fallback list does not recognize. Declared here so framework sync can key
// them. (Generator gap: internal-YAML specs should support a per-resource
// id_field; filed for retro.)
