...
func newSyncHintTestCmd() (*cobra.Command, *bytes.Buffer) {
	...
	cmd := &cobra.Command{Use: "municode-pp-cli"}
	cmd.SetErr(&stderr)
	return cmd, &stderr
}
...
func TestHintIfUnsynced_EmptySyncStateWritesHintToStderr(t *testing.T) {
	...
	if got := stderr.String(); !strings.Contains(got, "Run 'municode-pp-cli sync'") {
...
func TestHintIfStale_BackdatedSyncStateWritesHintToStderr(t *testing.T) {
	...
	got := stderr.String()
	if !strings.Contains(got, "older than --max-age=30m0s") || !strings.Contains(got, "Run 'municode-pp-cli sync'") {
