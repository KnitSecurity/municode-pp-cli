...
func TestMCPPathResolutionMatchesCLIResolverWithPlatformDefaults(t *testing.T) {
	...
	if want := filepath.Join(home, ".config", "municode-pp-cli", "config.toml"); cfg.Path != want {
	...
	if want := filepath.Join(home, ".local", "share", "municode-pp-cli", "data.db"); gotDB != want {
...
func TestMCPRegisterToolsPreservesTypedSpecialTools(t *testing.T) {
	s := server.NewMCPServer("municode", "test")
	RegisterTools(s)
