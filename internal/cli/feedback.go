...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MUNICODE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MUNICODE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/KnitSecurity/municode-pp-cli/feedback")
...
When ` + "`MUNICODE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MUNICODE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  municode-pp-cli feedback list
  municode-pp-cli feedback list --limit 5
  municode-pp-cli feedback list --json`,
