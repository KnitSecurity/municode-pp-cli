...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "municode-pp-cli",
		Short: `Municode CLI — Every Municode feature, plus an offline local mirror, real full-text search, change tracking, and citation tooling no o…`,
		Long: `Municode CLI — Every Municode feature, plus an offline local mirror, real full-text search, change tracking, and citation tooling no o…
...
Health check: run 'municode-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("municode-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'municode-pp-cli profile list')")
	...
	rootCmd.AddCommand(newCodestocCmd(flags))
	...
	rootCmd.AddCommand(newNovelXrefCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newContentPromotedCmd(flags))
	rootCmd.AddCommand(newVersionsPromotedCmd(flags))
