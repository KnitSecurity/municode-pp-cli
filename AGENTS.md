# Municode Printed CLI Agent Guide

This directory is a generated `municode-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
municode-pp-cli doctor --json
municode-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
municode-pp-cli which "<capability>" --json
municode-pp-cli <command> --help
```
...
```bash
municode-pp-cli <command> --help
municode-pp-cli <command> --dry-run --agent
```
